...
class ExchangeappModel extends Model
{
    ...
    public function get_items_info($varientsids,$arrayprms){
        ...
        //echo  $vids;
        $getexquery = "SELECT cg_exchange_return_order_products.return_exchange_reason,track_order_exchange_app.*
                       FROM track_order_exchange_app 
                       LEFT JOIN cg_exchange_return_order_products
                       ON cg_exchange_return_order_products.order_id = track_order_exchange_app.new_double_order_id 
                       AND cg_exchange_return_order_products.varient_id = track_order_exchange_app.varient_id
                       WHERE track_order_exchange_app.varient_id IN (".$vids.") 
                       AND track_order_exchange_app.new_double_order_id=? 
                       AND track_order_exchange_app.shop_url=?";
