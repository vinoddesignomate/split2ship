...
class FrontController extends BaseController
{
    ...
    public function get_product_details()
    {
        ...
        if ($this->request->getPost('vid') != 'undefined') {
                ...
                if (!empty($plan_details)) {
                    if ($plan_details[0]->plan_status == 'active' && $plan_details[0]->updated_sync_orders_count > 0) {
                        ...
                        if (!empty($getexlud)) {
                            ...
                            if (isset($get_user_choic[0]->choice_val) && $get_user_choic[0]->choice_val == 'spec_prodct') {
                            ...
                            } else if (isset($get_user_choic[0]->choice_val) && $get_user_choic[0]->choice_val == 'spec_coll') {
    ...
    function all_collection_productsget($shopname, $getcustpro, $cg_split_plan, $collectids)
    {
        ...
        if (!empty($getresylt)) {
            $gtbtncolor = $this->user_model->get_checkout_button_color($shopname);
            $partial_percentage = str_replace("%", "", $getresylt[0]->partial_percentage);
            $partperctg = $partial_percentage;
            $parttype = $getresylt[0]->partial_type;
            if (empty($gtbtncolor)) {
                $return_array = array(
                    "full_price" => 0,
                    "pro_pack" => $partperctg,
                    "partial_type" => $parttype,
                    "partial_price" => 0,
                    "var_qty" => 1,
                    "cart_button_id" => isset($gtbtncolor[0]->addcartbtn_cg) ? $gtbtncolor[0]->addcartbtn_cg : 'product-add-to-cart',
                    "cg_chkout_btn_class" => isset($gtbtncolor[0]->cg_chkout_btn_class) ? $gtbtncolor[0]->cg_chkout_btn_class : 'btn-checkout',
    ...
    {
        $gtbtncolor = $this->user_model->get_checkout_button_color($shopname);
        $partial_percentage = str_replace("%", "", $get_user_choic[0]->partial_value);
        $partperctg = $partial_percentage;
        $parttype = $get_user_choic[0]->partial_type;
        if (empty($gtbtncolor)) {
            $return_array = array(
                "full_price" => 0,
                "pro_pack" => $partperctg,
    ...
    public function create_draft_order()
    {
        ...
        if ($shopname == 'desinomatetest.myshopify.com') {
            $this->create_draft_order_designomate($body_data);
        } else {
    ...
    */
    public function add_collection_partial_cron()
    {
        ...
        //echo"get_lates_colection<pre>"; print_r($get_lates_colection); echo"</pre>";
    ...
    */
        ...
        */
        ...
        if (!empty($get_p_orders)) {
            foreach ($get_p_orders as $getnewords) {
                ...
                $get_details = $this->user_model->get_tokens($getnewords->shop_url);

                $getprietuleid = $this->common->rest_api('/admin/api/2023-07/orders/' . $getnewords->order_id . '.json', array(), 'GET', $get_details->access_token, $getnewords->shop_url);
                ...
                //set main order fulfilment by api
                $getprietuleid = $this->common->create_fulfilmentorders($get_details->access_token, $getnewords->shop_url, $fulfilarray);
                ...
                if ($matchingCode !== null) {
                    ...
                    $getorderarry = $this->common->create_actual_order($get_details->access_token, $getnewords->shop_url, $order_data);
                    $getlatsorder = json_decode($getorderarry);

                    if (isset($getlatsorder->order->id)) {

                        //reomve partial order if remaining COD order is create
                        $orders_data = array(
                            "order_id" => $getprietuleidrec['order']['id'],
                            "shop_url" => $getnewords->shop_url
                        );
                        ...
                        $track_double_order = array(
                            "orderid_paid" => $getprietuleidrec['order']['id'],
                            "order_id_cod" => $getlatsorder->order->id,
                            "order_number_cod" => $getprietuleidrec['order']['name'] . '-SplitOrder',
                            "shop_url" => $getnewords->shop_url,
                            "status" => 'success',
                        );
        ...
        $updateprorespo = array(
            "name" => "run create double order=" . json_encode($get_p_orders),
            "movement" => date('Y-m-d H:i')
        );
        $this->user_model->check_cron_ruinning_stst($updateprorespo);
        echo "done";
    ...
    public function removewebhookold()
    {
        ...
        if (isset($_GET['get_billing_info'])) {

            $get_register_webhook = $this->common->rest_api('/admin/api/2023-07/recurring_application_charges/23722623045.json', array(), 'GET', $get_details->access_token, $_GET['shop']);
            $get_register_webhookset = json_decode($get_register_webhook['body'], true);
