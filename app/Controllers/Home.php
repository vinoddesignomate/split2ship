...
class Home extends BaseController
{
        ...
        if (isset($_GET['shop'])) {
            ...
            if (array_key_exists('errors', $response_home)) {
                ...
                if ($_SERVER['HTTP_X_FORWARDED_FOR'] == '103.80.119.106' && $_GET['shop'] == 'desinomatetest.myshopify.com') {
                    ...
                    // $products = $this->common->rest_api($colcturl, array("collection_id" => 434528452912, "limit" => $cron_limit_set,"status"=>"active"), 'GET', $get_details->access_token, $_GET['shop']); 
                    ...
                    // $getprietuleid = $this->common->rest_api('/admin/api/2023-07/orders/5951897764188.json', array(), 'DELETE', $get_details->access_token, $_GET['shop']);
    ...
    public function collection_track_partial_percentage()
    {
        ...
        if ($this->request->getPost('update_per')) {
            ...
            if ($_REQUEST['shop'] == 'desinomatetest.myshopify.com') {
                ...
                $collectionc = $this->common->rest_api('/admin/api/2023-01/collects/count.json?collection_id=' . $this->request->getPost('colltion_change_partial_id') . '&status=ACTIVE ', array(), 'GET', $get_details->access_token, $_REQUEST['shop']);

                $collectconnt = json_decode($collectionc['body'], true);

                if (array_key_exists('errors', $collectconnt)) {

                    $smart_collections = $this->common->rest_api('/admin/api/2022-10/smart_collections/' . $this->request->getPost('colltion_change_partial_id') . '.json', array(), 'GET', $get_details->access_token, $_REQUEST['shop']);

                    $smart_collectionsget = json_decode($smart_collections['body'], true);
                    echo "<pre>";
                    print_r($smart_collectionsget);
                    echo "</pre>";
                    $countproducts = $smart_collectionsget['smart_collection']['products_count'];
                } else {
                ...
                if ($countproducts <= 100) {
                    ...
                    $get_updated_plan = $this->user_model->get_store_plane($_REQUEST['shop']); //get activated 
                    ...
                    if (!array_key_exists('errors', $product_list)) {
                        ...
                        if (!empty($product_list)) {
                            ...
                            if ($this->request->getPost('colltion_change_partial') != "") {
                                $partpercentg = str_replace("%", "", $this->request->getPost('colltion_change_partial'));
                            } else {
                                $partpercentg = 10;
                            }
                            if ($this->request->getPost('partiatype') == '') {
                                $coll_parttype = 'percentage';
                            } else {
                                $coll_parttype = $this->request->getPost('partiatype');
                            }
                            foreach ($product_list as $product) {
                                foreach ($product as $key => $value) {
    ...
    public function track_u_choic(){
        ...
        if($this->request->getPost('change_type') && $this->request->getPost('cg_choic_val') =='all_pro'){
