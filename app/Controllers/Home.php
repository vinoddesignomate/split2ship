...
class Home extends BaseController
{
    ...
    {
        $data = array();
        // $this->session->set('page', 'welcome');
        // $fieldata = file_get_contents('orderdsata2.json');
        // $jsdecoee =  json_decode($fieldata);
        // echo "orderdsata2<pre>";
        // print_r($jsdecoee);
        // echo "</pre>";
        // die();
        ...
        if (isset($_GET['shop'])) {
            ...
            if (array_key_exists('errors', $response)) {
                ...
                //     'item_name' => 'product one, product2, product 3',
                ...
                //     'to_pincode' => '201302',
                //     'to_address' => 'A - 130 , 91 SPRINGBOARD , SECTOR 63 , NOIDA , Gautam Buddha Nagar , Uttar Pradesh',
                //     'quantity' => 1,
                //     'invoice_value' => 400,
                //     'cod_amount' => 500,
                ...
                if ($this->request->getPost('assign_save')) {
                    ...
                    echo "<script>top.window.location='https://admin.shopify.com/store/" . $this->shope_name . "/apps/pay-x-now-rest-on-delivery/payxnowandrestondelivery/public/index.php/partial-products-list'</script>";
                    // exit();
                }
                ...
                $all_orders = $this->common->rest_api('/admin/api/2023-01/orders.json?status=any&order=updated_at asc', array(), 'GET', $get_details->access_token, $_GET['shop']);
                if (!empty($all_orders)) {
                    ...
                    foreach ($get_all_oders as $order) {
                        foreach ($order as $key => $value) {
                            ...
                            if (isset($value['shipping_address'])) {
                                $orders_data['shipping_address'] = $value['shipping_address']['address1'];
                                $orders_data['city'] = (isset($value['shipping_address']['city']) ? $value['shipping_address']['city'] : '');
                                $orders_data['state'] = (isset($value['shipping_address']['province']) ? $value['shipping_address']['province'] : '');
                                $orders_data['zip'] = (isset($value['shipping_address']['zip']) ? $value['shipping_address']['zip'] : '');
                                $orders_data['phone'] = (isset($value['shipping_address']['phone']) ? $value['shipping_address']['phone'] : '');
                                $orders_data['f_name'] = (isset($value['shipping_address']['first_name']) ? $value['shipping_address']['first_name'] : '');
                                $orders_data['l_name'] = (isset($value['shipping_address']['last_name']) ? $value['shipping_address']['last_name'] : '');
                                //$orders_data['email'] = (isset($value['shipping_address']['email']) ? $value['shipping_address']['email'] :'' );
                                $orders_data['country'] = (isset($value['shipping_address']['country']) ? $value['shipping_address']['country'] : '');
                            }
    ...
    public function check_subscribe()
    {
        ...
        if (empty($plan_details)) {
            echo "<script>top.window.location='https://admin.shopify.com/store/" . $this->shope_name . "/apps/pay-x-now-rest-on-delivery/payxnowandrestondelivery/public/index.php/price-plan'</script>";
            die();
        } else {
    ...
    {
        // echo "working function";
        //die();
        //echo "sessionval".$this->session->set('page');
        ...
        if ($this->request->getPost('assign_save')) {
            ...
            if (!empty($this->request->getPost('assign_pro'))) {
                ...
                if ($get_updated_plan[0]->updated_products_partial > $total_synproduct) {
                    ...
                    echo "<script>alert('Please upgrade the plan'); top.window.location='https://admin.shopify.com/store/" . $this->shope_name . "/apps/pay-x-now-rest-on-delivery/payxnowandrestondelivery/public/index.php/products-list?collectionparms=" . $this->request->getPost('get_coll') . "'</script>";
                }
            } else {
                echo "<script>top.window.location='https://admin.shopify.com/store/" . $this->shope_name . "/apps/pay-x-now-rest-on-delivery/payxnowandrestondelivery/public/index.php/products-list?collectionparms=" . $this->request->getPost('get_coll') . "'</script>";
            }
            //  echo "<script>top.window.location='https://admin.shopify.com/store/" . $this->shope_name . "/apps/pay-x-now-rest-on-delivery/public/index.php/products-list?collectionparms=" . $this->request->getPost('get_coll') . "'</script>";
            ...
            foreach ($get_all_oders as $order) {
                foreach ($order as $key => $value) {
                    ...
                    if (isset($value['shipping_address'])) {
                        ...
                        $orders_data['shipping_address'] = $value['billing_address']['address1'];
                        $orders_data['city'] = (isset($value['billing_address']['city']) ? $value['billing_address']['city'] : '');
                        $orders_data['state'] = (isset($value['billing_address']['province']) ? $value['billing_address']['province'] : '');
                        $orders_data['zip'] = (isset($value['billing_address']['zip']) ? $value['billing_address']['zip'] : '');
                        $orders_data['phone'] = (isset($value['billing_address']['phone']) ? $value['billing_address']['phone'] : '');
                        $orders_data['f_name'] = (isset($value['billing_address']['first_name']) ? $value['billing_address']['first_name'] : '');
                        $orders_data['l_name'] = (isset($value['billing_address']['last_name']) ? $value['billing_address']['last_name'] : '');
                        //$orders_data['email'] = (isset($value['shipping_address']['email']) ? $value['shipping_address']['email'] :'' );
                        $orders_data['country'] = (isset($value['billing_address']['country']) ? $value['billing_address']['country'] : '');
                    }
        ...
        if (isset($shiprocket_info[0]->enable_shipping_type) && $shiprocket_info[0]->enable_shipping_type == 'ship_roc') {
            ...
            foreach ($get_all_ordersnew as $set_orders) {
                ...
                if ($set_orders[0]['phone'] == "") {
                    ...
                    $phnum = str_replace(" ", "", $set_orders[0]['phone']);
                ...
                $create_custom = array(
                    ...
                    "billing_customer_name" => $set_orders[0]['cust_fname'],
                    "billing_last_name" => $set_orders[0]['cust_lname'],
                    "billing_address" => $set_orders[0]['shipping_address'],
                    "billing_city" => $set_orders[0]['shipping_city'],
                    "billing_pincode" => $set_orders[0]['zip'],
                    "billing_state" => $set_orders[0]['state'],
                    "billing_country" => $set_orders[0]['country'],
                    "billing_email" => $set_orders[0]['email'],
                    "billing_phone" => $phnum,
    ...
    function order_sync_delhivery($shiprocket_info)
    {
        ...
        foreach ($get_all_ordersdelhivery as $set_orders) {
            ...
            $postdata = 'format=json&data={
                "shipments": [
                    {
                        "add": "' . $set_orders[0]['shipping_address'] . '",
                        "address_type": "home",
                        "phone": "' . $phnum . '",
                        "payment_mode": "' . $shipping_pay_method . '",
                        "name": "' . $set_orders[0]['cust_fname'] . ' ' . $set_orders[0]['cust_lname'] . '",
                        "pin": "' . $set_orders[0]['zip'] . '",
                        "order": "' . $set_orders[0]['order_number'] . '",
                        "country": "' . $set_orders[0]['country'] . '",
    ...
    public function product_remove()
    {
        ...
        if (isset($_GET['id']) && $_GET['id'] != "") {
            ...
            echo "<script>alert('Product remove successfully'); top.window.location='https://admin.shopify.com/store/" . $this->shope_name . "/apps/pay-x-now-rest-on-delivery/payxnowandrestondelivery/public/index.php/partial-products-list'</script>";
        }
    }
    ...
    public function get_subscribe()
    {
        ...
        if (isset($_GET['plan']) && $_GET['plan'] != "" && $_GET['plan'] != "basic") {
            ...
            $get_subscribe = $this->common->rest_api('/admin/api/2022-10/recurring_application_charges.json', array("recurring_application_charge" => array("name" => $plane_name, "price" => $plane_price, "return_url" => 'https://admin.shopify.com/store/' . $this->shope_name . '/apps/pay-x-now-rest-on-delivery/payxnowandrestondelivery/public/index.php/return_url?shop=' . $_GET['shop'] . '&planname=' . $_GET['plan'] . $stype . '', "test" => true, "trial_days" => $freetrial)), 'POST', $get_details->access_token, $_GET['shop']);
    ...
    public function get_subscribe_return()
    {
        ...
        if (isset($_REQUEST['typu']) && $_REQUEST['typu'] == 'f') {
            ...
            echo "<script>alert('" . $_REQUEST['planname'] . " plan activated successfully'); top.window.location='https://admin.shopify.com/store/" . $this->shope_name . "/apps/pay-x-now-rest-on-delivery/payxnowandrestondelivery/public/index.php/price-plan'</script>";
            echo view('templates/apbrdgnew');
        }
    }
