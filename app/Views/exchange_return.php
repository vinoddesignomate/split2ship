                ...
                fetch('https://app.payxnowandrestondelivery.com/fetch-order-info', {
                    ...
                    .then(response => {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            infiohtm += '<input type="hidden" name="getid[]" id="setva_' + response[i]['varient_id'] + '" value=""><div class="boxesMain09"><div class="forIMGpurpose"><img src="' + response[i]['product_image'] + '" /></div><div class="forTextpurpose"><h4>' + response[i]['product_name'] + '</h4><h5>' + response[i]['product_price'] + ' x ' + response[i]['product_qty'] + '</h5></div><div class="reasonDefine"><h6>Non Reason: <span>Unfulfilled</span></h6></div><span><input id="' + response[i]['varient_id'] + '" type="checkbox" idattr="' + lpid + '" class="Polaris-Checkbox__Input" aria-invalid="false" role="checkbox" name="checkbox_name[]" aria-checked="false" value=""></span><select id="reason_' + lpid + '" style="display:none;" name="get_reason[]"><option>Select Reason</option><option>Arrive too late</option><option>Poor Quality</option><option>Looks Different</option><option>Does suit me</option><option>Parcel damaged on arrival</option><option>Poor Quality</option></select></div>';
                            lpid++;
                        }
